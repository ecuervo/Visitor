...
Begin VB.Form frmBuscar1 
   ...
   Begin VB.ListBox lstEmerge 
      ...
      ForeColor       =   &H00000000&
      Height          =   3630
      ItemData        =   "frmBuscar1.frx":0742
      Left            =   120
      List            =   "frmBuscar1.frx":0744
      TabIndex        =   1
      Top             =   480
      Width           =   6975
   End
   ...
   Begin VB.Image imgCerrar 
      Height          =   300
      Left            =   6795
      MouseIcon       =   "frmBuscar1.frx":0746
      MousePointer    =   99  'Custom
      Picture         =   "frmBuscar1.frx":0A50
      Top             =   120
      Width           =   300
   End
...
If KeyAscii = vbKeyReturn Then
    KeyAscii = 0
    If lstEmerge.Visible Then
        If lstEmerge.ListCount > 0 Then lstEmerge.ListIndex = 0
    End If
...
If Trim(txtNombre.Text) <> vbNullString Then
    lstEmerge.Clear
    sSql = "select top 1000 id,nombre + ' ' + apellidos as nombre  from tvisitantes_huella where nombre + ' ' + apellidos like '%" & Trim(txtNombre.Text) & "%' order by nombre"
    With objRst
